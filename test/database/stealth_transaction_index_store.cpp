...
BOOST_AUTO_TEST_SUITE(stealth_transaction_index_store_tests)
...
{
    stealth_transaction_index_store index;
...
{
    stealth_transaction_index_store index;
    auto store_result = index.store(1, 2, 3, 4);
    BOOST_REQUIRE(store_result);

    auto get_result = index.get(1);
    BOOST_REQUIRE(get_result);
...
BOOST_AUTO_TEST_CASE(get_binary_success)
{
    stealth_transaction_index_store index;
    uint32_t primary = 1;
    binary_type primary_binary(sizeof(uint32_t) * byte_bits,
        to_little_endian(primary));
...
{
    stealth_transaction_index_store index;
    uint32_t primary = 123;
    binary_type primary_binary(sizeof(uint32_t) * byte_bits, to_little_endian(primary));
...
{
    stealth_transaction_index_store index;
    auto result = index.store(1, 2, 3, 4);
    BOOST_REQUIRE(result);
...
{
    stealth_transaction_index_store index;
    auto result1 = index.store(1, 2, 3, 4);
    ...
    auto result2 = index.store(1, 12, 13, 0);
